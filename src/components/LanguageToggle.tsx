...
export default function LanguageToggle({ className = "" }: { className?: string }) {
  ...
  return (
    <div className={`flex items-center border border-[#e2e2e2] divide-x divide-[#e2e2e2] ${className}`}>
      {LOCALES.map((loc) => (
        <Link
          key={loc}
          ...
          className={`font-serif text-[12px] tracking-[0.08em] transition-colors px-2.5 py-1 ${
            loc === current
